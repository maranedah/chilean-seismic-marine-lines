...
/graphify <path> --neo4j                              # generate data/graphify-out/cypher.txt for Neo4j
...
1. **Persistent graph** - relationships are stored in `data/graphify-out/graph.json` and survive across sessions. Ask questions weeks later without re-reading everything.
...
Path('data/graphify-out/GRAPH_REPORT.md').write_text(report)
to_json(G, communities, 'data/graphify-out/graph.json')
...
n = to_obsidian(G, communities, 'data/graphify-out/obsidian', community_labels=labels or None, cohesion=cohesion)
print(f'Obsidian vault: {n} notes in data/graphify-out/obsidian/')

to_canvas(G, communities, 'data/graphify-out/obsidian/graph.canvas', community_labels=labels or None)
print('Canvas: data/graphify-out/obsidian/graph.canvas - open in Obsidian for structured community layout')
print()
print('Open data/graphify-out/obsidian/ as a vault in Obsidian.')
...
if G.number_of_nodes() > 5000:
    ...
    to_html(G, communities, 'data/graphify-out/graph.html', community_labels=labels or None)
...
to_cypher(G, 'data/graphify-out/cypher.txt')
print('cypher.txt written - import with: cypher-shell < data/graphify-out/cypher.txt')
"
```
...
to_svg(G, communities, 'data/graphify-out/graph.svg', community_labels=labels or None)
...
to_graphml(G, communities, 'data/graphify-out/graph.graphml')
...
```powershell
python -m graphify.serve data/graphify-out/graph.json
```
...
```json
{
  "mcpServers": {
    "graphify": {
      "command": "python",
      "args": ["-m", "graphify.serve", "/absolute/path/to/data/graphify-out/graph.json"]
    }
  }
}
```
...
detection = json.loads(Path('.graphify_detect.json').read_text())
result = run_benchmark('data/graphify-out/graph.json', corpus_words=detection['total_words'])
...
cost_path = Path('data/graphify-out/cost.json')
if cost_path.exists():
...
Remove-Item -ErrorAction SilentlyContinue data/graphify-out/.needs_update
```

Tell the user (omit the obsidian line unless --obsidian was given):
```
Graph complete. Outputs in PATH_TO_DIR/data/graphify-out/
...
# Load existing graph
existing_data = json.loads(Path('data/graphify-out/graph.json').read_text())
...
Before the merge step, save the old graph: `Copy-Item data/graphify-out/graph.json .graphify_old.json`
...
Skip Steps 1–3. Load the existing graph from `data/graphify-out/graph.json` and re-run clustering:
...
data = json.loads(Path('data/graphify-out/graph.json').read_text())
...
python -c "
from pathlib import Path
if not Path('data/graphify-out/graph.json').exists():
...
Load `data/graphify-out/graph.json`, then:
...
save_query_result(
    question='QUESTION',
    answer='ANSWER',
    memory_dir=Path('data/graphify-out/memory'),
    query_type='query',
    source_nodes=SOURCE_NODES,  # list of node labels cited, or []
)
print('Query result saved to data/graphify-out/memory/')
"
```
...
print('Path result saved to data/graphify-out/memory/')
"
```
...
print('Explanation saved to data/graphify-out/memory/')
"
```
...
- **Docs, papers, or images:** writes a `data/graphify-out/needs_update` flag and prints a notification to run `/graphify --update` (LLM semantic re-extraction required).
