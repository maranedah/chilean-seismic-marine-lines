...
Usage:
    python download_pdfs.py --all --email you@example.com
    python download_pdfs.py --all --dry-run
    python download_pdfs.py --paper bangs_2020_basal_accretion_jgr --email you@example.com
"""
...
def download_pdf(url: str, dest: Path, dry_run: bool) -> bool:
    ...
    try:
        r = SESSION.get(url, timeout=60, stream=True, allow_redirects=True, verify=verify)
        r.raise_for_status()
...
def main() -> None:
    ...
    for paper in papers:
        ...
        if download_pdf(pdf_url, dest, args.dry_run):
            ok += 1
        else:
            failed += 1
